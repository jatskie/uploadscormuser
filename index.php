...
require_login();
admin_externalpage_setup('tooluploadscormuser');
require_capability('moodle/site:uploadusers', context_system::instance());

$struserrenamed             = get_string('userrenamed', 'tool_uploadscormuser');
...
$struserupdated             = get_string('useraccountupdated', 'tool_uploadscormuser');
...
$struseruptodate            = get_string('useraccountuptodate', 'tool_uploadscormuser');
...
$struserdeleted             = get_string('userdeleted', 'tool_uploadscormuser');
...
$strno                      = get_string('no');
$stryesnooptions = array(
		0	=> $strno, 
		1	=> $stryes);

$returnurl = new moodle_url('/admin/tool/uploadscormuser/index.php');
...
if (empty($iid)) 
{
    $mform1 = new admin_uploadscormuser_form1();

    if ($formdata = $mform1->get_data()) 
    {
        $iid = csv_import_reader::get_new_iid('uploadscormuser');
        ...
        // test if columns ok
        $filecolumns = us_validate_user_upload_columns($cir, $STD_FIELDS, $PRF_FIELDS, $returnurl);
    ...
    else 
    {
        ...
        //echo $OUTPUT->heading_with_help(get_string('uploadscormusers', 'tool_uploadscormuser'), 'uploadscormusers', 'tool_uploadscormuser');
...
$mform2 = new admin_uploadscormuser_form2(
		null, 
		array(
			'columns' => $filecolumns, 
			'data' => array(
...
else if ($formdata = $mform2->get_data()) 
{
    // Print the header
    echo $OUTPUT->header();
    echo $OUTPUT->heading(get_string('uploadscormusersresult', 'tool_uploadscormuser'));

    $optype = $formdata->ustype;
    $updatetype        = isset($formdata->usupdatetype) ? $formdata->usupdatetype : 0;
    $createpasswords   = (!empty($formdata->uspasswordnew) and $optype != US_USER_UPDATE);
    $updatepasswords   = (!empty($formdata->uspasswordold)  and $optype != US_USER_ADDNEW and $optype != US_USER_ADDINC and ($updatetype == US_UPDATE_FILEOVERRIDE or $updatetype == US_UPDATE_ALLOVERRIDE));
    $allowrenames      = (!empty($formdata->usallowrenames) and $optype != US_USER_ADDNEW and $optype != US_USER_ADDINC);
    $allowdeletes      = (!empty($formdata->usallowdeletes) and $optype != US_USER_ADDNEW and $optype != US_USER_ADDINC);
    $allowsuspends     = (!empty($formdata->usallowsuspends));
    $bulk              = $formdata->usbulk;
    $noemailduplicates = $formdata->usnoemailduplicates;
    $standardusernames = $formdata->usstandardusernames;
    $resetpasswords    = isset($formdata->usforcepasswordchange) ? $formdata->usforcepasswordchange : US_PWRESET_NONE;
    ...
    $cohorts        = array();
    $rolecache      = us_allowed_roles_cache(); // roles lookup cache
    $manualcache    = array(); // cache of used manual enrol plugins in each course
    $supportedauths = us_supported_auths(); // officially supported plugins that are enabled
    ...
    while ($line = $cir->next()) 
    {
        ...
        if ($optype == US_USER_ADDNEW or $optype == US_USER_ADDINC) 
        {
        ...
        if ($user->mnethostid == $CFG->mnet_localhost_id) 
        {
            ...
            if ($existinguser and $optype == US_USER_ADDINC) 
            {
                $user->username = us_increment_username($user->username);
                $existinguser = false;
            }
        ...
        {
            if (!$existinguser or $optype == US_USER_ADDINC) 
            {
                $upt->track('status', get_string('errormnetadd', 'tool_uploadscormuser'), 'error');
                $userserrors++;
                continue;
            }
        ...
        foreach ($STD_FIELDS as $field) {
            ...
            if (isset($formdata->$field)) {
                // process templates
                $user->$field = us_process_template($formdata->$field, $user);
        ...
        if ($existinguser) {
            ...
            if ($updatetype != US_UPDATE_NOCHANGES and !$remoteuser) {
                ...
                foreach ($allcolumns as $column) {
                    ...
                    if ($updatetype == US_UPDATE_MISSING) {
                        if (!is_null($existinguser->$column) and $existinguser->$column !== '') {
                            continue;
                        }
                    } else if ($updatetype == US_UPDATE_ALLOVERRIDE) {
                        // we override everything

                    } else if ($updatetype == US_UPDATE_FILEOVERRIDE) {
            ...
            if ($remoteuser) {
                ...
                if ($updatepasswords) {
                    ...
                    if ($resetpasswords == US_PWRESET_ALL or ($resetpasswords == US_PWRESET_WEAK and $weak)) {
                        if ($weak) {
                            $weakpasswords++;
            ...
            if ($doupdate or $existinguser->password !== $oldpw) {
                ...
                if (!$remoteuser) {
                    ...
                    $existinguser = us_pre_process_custom_profile_data($existinguser);
                    // save custom profile fields data from csv file
                    profile_save_data($existinguser);
                }

                if ($bulk == US_BULK_UPDATED or $bulk == US_BULK_ALL) {
            ...
            if ($isinternalauth) {
                if (empty($user->password)) {
                    if ($createpasswords) {
                        ...
                        $upt->track('password', '', 'normal', false);
                        $upt->track('password', get_string('uspasswordcron', 'tool_uploadscormuser'), 'warning', false);
                    } else {
                        $upt->track('password', '', 'normal', false);
            ...
            // pre-process custom profile menu fields data from csv file
            $user = us_pre_process_custom_profile_data($user);
            ...
            if ($bulk == US_BULK_NEW or $bulk == US_BULK_ALL) {
                if (!in_array($user->id, $SESSION->bulk_users)) {
                    $SESSION->bulk_users[] = $user->id;
                }
            }
            ...
            if ($courseid == SITEID) {
                ...
                if (!empty($user->{'role'.$i})) {
                    ...
                    if ($addtype < 1 or $addtype > 3) {
                        ...
                        continue;
                    } else if (empty($formdata->{'uslegacy'.$addtype})) {
                        continue;
                    } else {
                        $rid = $formdata->{'uslegacy'.$addtype};
                    }
                } else {
    ...
    if ($optype != US_USER_UPDATE) {
        echo get_string('userscreated', 'tool_uploadscormuser').': '.$usersnew.'<br />';
    }
    if ($optype == US_USER_UPDATE or $optype == US_USER_ADD_UPDATE) {
        echo get_string('usersupdated', 'tool_uploadscormuser').': '.$usersupdated.'<br />';
    }
    if ($allowdeletes) {
        echo get_string('usersdeleted', 'tool_uploadscormuser').': '.$deletes.'<br />';
        echo get_string('deleteerrors', 'tool_uploadscormuser').': '.$deleteerrors.'<br />';
    }
    if ($allowrenames) {
        echo get_string('usersrenamed', 'tool_uploadscormuser').': '.$renames.'<br />';
        echo get_string('renameerrors', 'tool_uploadscormuser').': '.$renameerrors.'<br />';
    }
    if ($usersskipped) {
        echo get_string('usersskipped', 'tool_uploadscormuser').': '.$usersskipped.'<br />';
    }
    echo get_string('usersweakpassword', 'tool_uploadscormuser').': '.$weakpasswords.'<br />';
    echo get_string('errors', 'tool_uploadscormuser').': '.$userserrors.'</p>';
    echo $OUTPUT->box_end();
...
echo $OUTPUT->heading(get_string('uploadscormuserspreview', 'tool_uploadscormuser'));
...
while ($linenum <= $previewrows and $fields = $cir->next()) {
    ...
    $noerror = us_check_custom_profile_data($rowcols) && $noerror;
...
$table->summary = get_string('uploadscormuserspreview', 'tool_uploadscormuser');
$table->head = array();
$table->data = $data;

$table->head[] = get_string('uscsvline', 'tool_uploadscormuser');
foreach ($filecolumns as $column) {
    $table->head[] = $column;
}
